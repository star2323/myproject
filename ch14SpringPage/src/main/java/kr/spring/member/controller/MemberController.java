...
public class MemberController {
	...
	BindingResult result) {
		...
		memberCommand.setPasswd(
				cipherAES.encrypt(memberCommand.getPasswd()));
	...
	public String submitChangePassword(
		...
		MemberCommand member = memberService.selectMember(
				                     memberCommand.getId());
		//사용자가 입력한 현재 비밀번호와 DB의 현재 비밀번호 일치
		//여부 체크
		if(!member.getPasswd().equals(
				 cipherAES.encrypt(memberCommand.getOld_passwd()))) {
